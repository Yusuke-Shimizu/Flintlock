...
namespace Tutorial.Utility
{
	...
	public class SimpleSprite
	{
		...
		/// テクスチャ座標を指定します。
		/// </summary>
		public void SetTextureCoord2(float x0, float y0, float x1, float y1)
		{
			texcoords[0] = x0;	// left top u
			texcoords[1] = y0; // left top v
			
			texcoords[2] = x0;	// left bottom u
			texcoords[3] = y1;	// left bottom v
			
			texcoords[4] = x1;	// right top u
			texcoords[5] = y0;	// right top v
			
			texcoords[6] = x1;	// right bottom u
			texcoords[7] = y1;	// right bottom v
		}
